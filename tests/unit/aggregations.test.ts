...
beforeAll(async () => {
  ...
  // Income: July 2026 — H1 $1,000 USD, H2 ₡125,000 CRC (actual), plus a planned entry.
  await prisma.incomeEntry.createMany({
    data: [
      { userId, year: 2026, month: 7, period: "H1", amountMinor: 100000, currency: "USD" },
      { userId, year: 2026, month: 7, period: "H2", amountMinor: 12500000, currency: "CRC" },
  ...
  // Expenses: July — $100 USD food (H1), ₡25,000 CRC rent (H2); August — $50 USD.
...
describe("income totals (R3)", () => {
  ...
  it("reports null totals when a rate is missing (R5)", async () => {
    const data = await getIncomeForMonth(userId, 2026, 7, "CRC", { usdToCrc: null });
    expect(data.totalH1).toBeNull();
  });
});

describe("expense listing and conversion (R4, R5)", () => {
  it("shows native and converted values for USD and CRC expenses", async () => {
    const data = await getExpenses(userId, { year: 2026, month: 7 }, "CRC", RATES);
    expect(data.expenses).toHaveLength(2);
    const usd = data.expenses.find((e) => e.currency === "USD")!;
    const crc = data.expenses.find((e) => e.currency === "CRC")!;
    expect(usd.amountMinor).toBe(10000);
    expect(usd.convertedMinor).toBe(5000000);
    expect(crc.convertedMinor).toBe(2500000);
  ...
  it("supports USD reporting for mixed expenses", async () => {
    const data = await getExpenses(
      userId,
      { year: 2026, month: 7, categoryId: foodId },
      "USD",
    ...
    expect(data.expenses[0].convertedMinor).toBe(10000);
  });
});
...
describe("plan matrix totals (R6)", () => {
  it("computes row, column, and grand totals", async () => {
    ...
    expect(food.rowTotal).toBe(30000);
    expect(rent.rowTotal).toBe(30000);
    expect(matrix.columnTotals[0]).toBe(40000);
    expect(matrix.columnTotals[1]).toBe(20000);
  ...
  it("joins actual spend per category and month", async () => {
    const matrix = await getPlanMatrix(userId, 2026, "CRC", RATES);
    const food = matrix.rows.find((r) => r.categoryName === "Food")!;
    expect(food.actual[6]).toBe(5000000);
  });
});
...
describe("monthly overview (R7, R12)", () => {
  it("computes earned, spent, saved, remaining for July 2026", async () => {
    ...
    expect(o.saved).toBe(70_000);
    expect(o.remaining).toBe(62_500_000 - 7_500_000 - 70_000);
    expect(o.plannedExpensesMinor).toBe(0);
  ...
  it("renders zeros for an empty month", async () => {
    ...
    expect(o.saved).toBe(70_000);
    expect(o.remaining).toBe(-70_000);
    expect(o.plannedExpensesMinor).toBe(0);
  });
});
