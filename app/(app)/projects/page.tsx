...
import { MobileProjects } from "@/components/projects/mobile-projects";
import { AddProjectForm, ProjectCard, type ProjectCardData } from "./project-forms";

export const dynamic = "force-dynamic";

function toCardData(
  project: Awaited<ReturnType<typeof getProjectsView>>["projects"][number],
  reportingCurrency: ProjectCardData["reportingCurrency"],
...
export default async function ProjectsPage() {
  ...
  const cardProjects = view.projects.map((project) =>
    toCardData(project, settings.reportingCurrency, view.projectionPossible)
  );
  const activeCount = view.projects.filter((project) => !project.completedAt).length;
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <MobileProjects
        ...
        totalCount={view.projects.length}
        postLifetimeMinor={view.postLifetimeMinor}
